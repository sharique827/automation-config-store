export async function select_2_DefaultGenerator(
  ...
  const items =
    sessionData?.on_select_1_items
      ?.filter((item: any) => item?.parent_item_id)
      .map((item: any) => {
  ...
  const mappedFulfillments = existingPayload.message.order.items.map(
    (_i: any, index: number) => {
  ...
  const fulfillments = fulfillment?.map(
    (item: any) => {
      if (item.type === "TICKET") {
        const seatGrid = item.tags.find(
          (t: any) => t.descriptor?.code === "SEAT_GRID"
